# Author: Mei-Ren Ke
# Update: 2022/08/18
# 用途：讓User以投票的方式決定圖片風格，輸出一個csv檔（vote.csv）
...
yourpath = input("輸入放vote.csv的資料夾路徑 如：/Users/yourname/Desktop/filename： ")
yourpath = yourpath + "/vote.csv"

if os.path.isfile(yourpath)== False:
    with open(yourpath, 'a+', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(['圖片名稱', '田園鄉村', '浪漫柔和', '懷舊復古', '灰暗憂鬱', '繽紛色彩'])

if os.path.isfile(yourpath):
    if os.path.getsize(yourpath)== False:
        with open(yourpath, 'a+', newline='') as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow(['圖片名稱', '田園鄉村', '浪漫柔和', '懷舊復古', '灰暗憂鬱', '繽紛色彩'])
